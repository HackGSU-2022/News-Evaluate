...
def main():
    ...
    tokens = common_tags(summary)
    result = classifier(summary)
    similar_article = top_similiar_article(user_url, article)
    st.write("Here is your information: \n")
    st.write("Summary: ", summary)
    st.write("")
    st.write("The text is labeled as: ", {list(result[0].values())[0]}, "with a score of: ", {round(list(result[0].values())[1] * 100,4)}," %")
    st.write("\nThe Comomon Tags of this article were: ", tokens)
    st.write("\nSimilar Article: " , similar_article)
